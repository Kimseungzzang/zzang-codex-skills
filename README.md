...
- Installs shared scripts into `~/.claude/scripts/`
...
Both `zzang-claude-skills` and `zzang-codex-skills` read/write the same `~/.claude/zzang-ctx/` directory and the same GitHub sessions repo.
...
Scripts install to `~/.claude/scripts/` and are shared between both tools:

| Script | Hook | Purpose |
|--------|------|---------|
| `task-log.sh` | PostToolUse | Log every tool use to `task-log.md` |
